...
import { useIf as Iff } from 'bresnow_utility-react-hooks' // useIf is essentially useEffect with a conditional array
/**
...
export function EnhancedScripts(opts?: { manualInjection: ScriptDescriptor[] }) {
  ...
  Iff([scripts.length > 0], () => {
    {
      scripts.map(({ id, ...props }) => {
        let target = document.getElementById(id) //can never escape good ol vanilla js
        if (props.markup) {
